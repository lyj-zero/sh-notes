...
start(){
	...
	/sbin/tc class add dev $IN parent 1:1 classid 1:2 cbq bandwidth 3Mbit rate 2Mbit maxburst 20 allot 1514 prio 1 avpkt 1000 cell 8 weight 200Kbit split 1:0 bounded
	...
	/sbin/tc class add dev $IN parent 1:1 classid 1:3 cbq bandwidth 3Mbit rate 1500Kbit maxburst 20 allot 1514 prio 2 avpkt 1000 cell 8 weight 100Kbit split 1:0
	...
	/sbin/ip route add 192.168.0.2 dev $IN via 192.168.0.1 realm 2

	#2） 发往主机192.168.1.30的数据包通过分类3转发(分类3的速率1500Kbit)
	/sbin/ip route add 192.168.0.3 dev $IN via 192.168.0.1 realm 3
	/sbin/ip route add 192.168.0.4 dev $IN via 192.168.0.1 realm 3
	/sbin/ip route add 192.168.0.5 dev $IN via 192.168.0.1 realm 3
	#3）发往子网192.168.1.0/24的数据包通过分类4转发(分类4的速率1000Kbit)
	/sbin/ip route add 192.168.0.0/24 dev $IN via 192.168.0.1 realm 4
...
stop(){
	...
	/sbin/ip route del 192.168.0.0/24 dev $IN via 192.168.0.1 realm 4
	/sbin/ip route add 192.168.0.0/24 dev eth1  proto kernel  scope link  src 192.168.0.1
	/sbin/ip route del 192.168.0.2 dev $IN via 192.168.0.1 realm 2
	#2） 发往主机192.168.1.30的数据包通过分类3转发(分类3的速率1500Kbit)
	/sbin/ip route del 192.168.0.3 dev $IN via 192.168.0.1 realm 3
	/sbin/ip route del 192.168.0.4 dev $IN via 192.168.0.1 realm 3
	/sbin/ip route del 192.168.0.5 dev $IN via 192.168.0.1 realm 3
	#3）发往子网192.168.1.0/24的数据包通过分类4转发(分类4的速率1000Kbit)
...
status() {
	...
	/sbin/tc -s qdisc ls dev $IN
	echo "2.show class $IN:----------------------------------------------"
	N1=`/sbin/tc class ls dev $IN | wc -l`
	if [ $N1 == 0 ];then
		echo "NULL, OFF Limiting "
	else
		/sbin/tc -s class ls dev $IN
		echo "It work"
	fi
}
#show help
